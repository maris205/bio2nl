...
The emergence of foundation models has made \emph{cross-domain transfer}---the reuse of
representations learned in one domain to solve tasks in another---one of their most consequential
and least understood properties. A particularly surprising instance has been reported between
...
training corpora may already contain biological data, confounding genuine structural transfer with
memorization \cite{lin2023,elnaggar2021}. Whether the reverse transfer exists, and whether
...
\begin{figure}[t]
\centering
\includegraphics[width=0.78\textwidth]{figures/fig1_directionality.pdf}
...
with model scale; reverse transfer (biology\,$\rightarrow$\,language) is weak, does not exceed
matched-token controls, and decays with scale. The asymmetry is the central finding.}
\label{fig:overview}
\end{figure}
...
nested-bracket (Dyck) task that remains hard at 355M, protein-CPT shows no advantage over its
shuffled control at either scale. Reverse structural benefit is thus real only in a narrow,
...
transfer to language. Fine-tuned on PAWS-X, ESM-2 (8M) reaches $0.641$---above the $0.545$
...
occupy a shared representational subspace by computing linear CKA \cite{zhang2019cka} between a
model's per-layer representations of PAWS-X pairs and of protein pairs. Across base GPT-2,
