...
converse question---\emph{is structural transfer between language and biology directional?}---and
answer it systematically. We reproduce forward transfer (language\,$\rightarrow$\,biology), then
evaluate the reverse direction across fine-tuning, iso-token continued pretraining, model scaling,
two biological foundation-model families (ESM-2, ProtBERT), and adversarial synthetic tasks. Reverse
transfer is consistently weak: it never exceeds matched-token controls, does not scale, and does not
generalize. In an architecture-matched $2{\times}2$ analysis on models with \emph{known} training
data---eliminating the pretraining-contamination confound that clouds large-model studies---a
language model retains far more competence when moved to biology (off-domain drop $0.08$) than a
biological model moved to language (drop $0.36$). Scaling widens this gap: language$\rightarrow$biology
transfer strengthens with size while biology$\rightarrow$language transfer decays toward chance, in
two independent protein-model families. Shared structural regularities between language and
biological sequences thus do \emph{not} imply symmetric representational transfer, revealing an
intrinsic directionality in cross-domain foundation-model learning.
\end{abstract}
...
\subsection{A unified protocol for measuring transfer in both directions}
...
\subsection{Part I: Forward transfer (language $\rightarrow$ biology)}
...
\subsection{Part II: Reverse transfer (biology $\rightarrow$ language) is weak}
...
\subsection{Part III: The transfer is directional}
...
\subsection{Part IV: Mechanistic origin of the asymmetry}
